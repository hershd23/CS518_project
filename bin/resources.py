...
def download_embedding():
    ...
    if not os.path.exists(lib.get_conf('embedding_path')):
        logging.warning('embedding_path does not exist. Extracting embedding.')
        logging.info(
            'Extracting embedding data from: {} to: {}'.format(embedding_downloaded_path, lib.get_conf('embedding_path')))

    with gzip.open(embedding_downloaded_path, 'rb') as zipped, \
            open(lib.get_conf('embedding_path'), 'wb') as unzipped:
        for line in zipped:
            unzipped.write(line)

    logging.info('Embeddings available at: {}'.format(lib.get_conf('embedding_path')))
...
def create_embedding_matrix():
    ...
    glove_file = '../resources/glove.6B.100d.txt'
    tmp_file = '../resources/gensim_glove.txt'
    new_file = glove2word2vec(glove_file, tmp_file)
    # Get word weights from file via gensim
    model = gensim.models.KeyedVectors.load_word2vec_format('../resources/gensim_glove.txt', binary=False)
